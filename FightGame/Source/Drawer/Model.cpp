...
void Model::DrawAlphaBlend(Texture* texture1, Texture* texture2, int meshNo)
{
	// シェーダー設定
	m_pVS->Bind();
	m_pPS->Bind();
	...
	{
		if (isAutoTexture) {
			m_pPS->SetTexture(0, texture1);
			m_pPS->SetTexture(1, texture2);
		}
		m_meshes[i].pMesh->Draw();
	}
}
...
const void* Model::LoadAssimpScene(const char* file)
{
	...
	if (m_loadFlip == Flip::ZFlip || m_loadFlip == Flip::XZFlip) {
