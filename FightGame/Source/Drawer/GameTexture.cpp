...
HRESULT Texture::CreateFromAssimp(const aiTexture* pTexture)
{
	if (!pTexture || !pTexture->pcData) {
		return E_FAIL;
	}

	// 圧縮形式（FBX Embed Media 等）
	if (pTexture->mHeight == 0) {
		DirectX::TexMetadata mdata;
		DirectX::ScratchImage image;
		HRESULT hr = E_FAIL;

		if (pTexture->CheckFormat("tga")) {
			hr = DirectX::LoadFromTGAMemory(
				pTexture->pcData, pTexture->mWidth, &mdata, image);
		}
		else {
			hr = DirectX::LoadFromWICMemory(
				pTexture->pcData, pTexture->mWidth,
				DirectX::WIC_FLAGS::WIC_FLAGS_IGNORE_SRGB, &mdata, image);
		}
		if (FAILED(hr)) {
			return hr;
		}
		...
		hr = CreateShaderResourceView(
			GetDevice(), image.GetImages(), image.GetImageCount(), mdata, &m_pSRV);
		if (SUCCEEDED(hr)) {
			m_width = static_cast<UINT>(mdata.width);
			m_height = static_cast<UINT>(mdata.height);
		}
		return hr;
	}

	// 非圧縮 ARGB8888（aiTexel: b,g,r,a）
	return Create(
		DXGI_FORMAT_R8G8B8A8_UNORM,
		pTexture->mWidth, pTexture->mHeight, pTexture->pcData);
}
