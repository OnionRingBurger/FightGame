---
name: custom-ecs
description: 自作ECS(共通基盤)の仕様とSystemの書き方?��?Chunk/Entity/ComponentView/ComponentHandle/SystemContextを使?��?コード�??��読み書き、Systemの新規作�??��、移動系(TransformSystem・Pose・MotionResult?��?)の変更を行うときに使?��??��?ECS、System、Component、Entity、移動�??��?��?、座標更新に関する依�??��で参�??��する?��?
---

# 自作ECS

自作�??��ECS共通基盤。�??���?ダは `include/ECS/` 配�?(`Core` / `Component` / `System` / `World`)、実�?は `Source/ECS/` 配下に同じ構�??��で配置される�?
...
- **Chunk**: EntityとComponentの管?��??��?。SystemはChunk経由でComponentにアクセスする
- **World**(`include/ECS/World/`): ?��?シーン(GameWorld?��?)がUpdate?��?でSystemを呼び出し�??に実行する�?**Systemの実行�??はWorldの呼び出し�??がすべて**
...
{
	// ?��?要なComponentを�??��て持つEntityを�??��挙するView
...
早?��?/?��??��? の振り�??け�??�� `PosePosState` / `PoseRotState` のenum値で行う(RAIL・LEAP・NONEは先、CAMERA・FOLLOW・LOOK等�??��?��?延)?��?
...
4. プロジェクト固有�??��追?��?手�??(System.hへのinclude追?��??��?)はプロジェクト�??��Rulesに従う
